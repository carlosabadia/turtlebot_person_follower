from faceNet.selfieSegmentation import MPSegmentation
import cv2
import stow
import typing
import numpy as np
...
class Engine:
    ...
    def __init__(
        ...
        self.turtlebot_video_path = turtlebot_video_path
    ...
    def custom_processing(self, frame) -> np.ndarray:
    ...
    def process_image(
        ...
        if image is not None and isinstance(image, str):
            if not stow.exists(image):
                raise Exception(
                    f"Given image path doesn't exist {self.image_path}")
            else:
    ...
    def process_webcam(self, return_frame: bool = False) -> typing.Union[None, np.ndarray]:
        ...
        while cap.isOpened():
            ...
            raise Exception(
                f"Webcam with ID ({self.webcam_id}) can't be opened")
    ...
    def process_video(self) -> None:
        ...
        if not stow.exists(self.video_path):
            raise Exception(
                f"Given video path doesn't exists {self.video_path}")
        ...
        if not cap.isOpened():
            raise Exception(
                f"Error opening video stream or file {self.video_path}")
        ...
        output_path = self.video_path.replace(
            f".{stow.extension(self.video_path)}", f"_{self.output_extension}.mp4")
        ...
        for fnum in tqdm(range(frames)):
            ...
            # frame = self.custom_processing(self.flip(frame)) # flip is not needed for video
    ...
    def turtlebot_camera(self, data):
        """Process turtlebot camera stream for given topic
        """
        np_arr = np.fromstring(
            data.data, np.uint8)  # convert compressed image to numpy array
