...
pub = rospy.Publisher('/center', center, queue_size=10)
...
class FaceNet:
    ...
    def __init__(
        ...
        #rospy.init_node('nodocenter', anonymous=True)
    ...
    def detect_save_faces(self, image: np.ndarray, output_dir: str = "faces", name: str = None):
        ...
        for index, crop in enumerate(face_crops):
            output_path = stow.join(output_dir, f"{name}_{str(index)}.png")
    ...
    def draw(self, image: np.ndarray, face_crops: dict):
        ...
        if not face_crops.values(): # if face_crops is empty
            mensaje.person = "Unknown" 
        for value in face_crops.values(): # if face_crops is not empty
            t, l, b, r = value["tlbr"]
            if value['name'] == self.person_name: # if the person is the person we are looking for
                mensaje.person = value["name"] # name of the person
                width = l + ((r-l)/2)  # center of the face (x)
                height = t + ((b-t)/2) # center of the face (y)
                mensaje.x = (int(width)) 
                mensaje.y = (int(height))
        ...
        pub.publish(mensaje) # publish the message
        return image
    ...
    def __call__(self, frame: np.ndarray) -> np.ndarray:
        ...
        for key, value in face_crops.items():
            ...
            print("Face recognized:", face_crops[key]["name"])
